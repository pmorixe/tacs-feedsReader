...
class TypeRSS_2_0 extends TypeAbstract {
					...
					if (evalue != null) {
						...
						if (ensuri.equals(nsuri)) {
							if (ename.equals("item")) {
								// FeedItem!
								RSSFeedItem item = handleItem(source, element);
								if (item != null) {
									feed.addItem(item);
								}
	 ...
	 * Item parser.
	 */
	private static RSSFeedItem handleItem(URL source, RawElement rawItem) {
		// Namespace URI.
	 ...
	 * Attachments handler.
	 */
	private static RSSFeedEnclosure handleEnclosure(RawElement rawEnclosure) {
		// Namespace URI.
