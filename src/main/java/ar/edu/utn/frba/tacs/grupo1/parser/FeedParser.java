...
public class FeedParser {
  ...
  public Feed readFeed() throws UnsupportedRSSFeedException, RSSFeedIOException, RSSFeedXMLParseException {
    ...
    Feed domainFeed = new Feed(header.getTitle(), header.getLink().toString(), header.getDescription(),
        header.getLanguage(), null, String.valueOf(header.getPubDate()));
    ...
    for (int index = 0; index < countItem; index++) {
      ...
      list.add(new Entry(item.getTitle(), item.getDescriptionAsText(), item.getLink().toString(), null, item
          .getGUID()));
    }
